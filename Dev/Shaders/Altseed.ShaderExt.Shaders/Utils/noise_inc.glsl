...
float valueNoise(vec3 st)
{
    ...
    float v000_100 = lerp1(v000, v100, u.x);
    float v010_110 = lerp1(v010, v110, u.x);
    float v001_101 = lerp1(v001, v101, u.x);
    float v011_111 = lerp1(v011, v111, u.x);
    return lerp1(
        lerp1(v000_100, v010_110, u.y),
        lerp1(v001_101, v011_111, u.y),
        u.z
    );
}
...
float perlinNoise(vec3 st)
{
    ...
    float a = lerp1( dot( v000, f - vec3(0,0,0) ), dot( v100, f - vec3(1,0,0) ), u.x );
    float b = lerp1( dot( v010, f - vec3(0,1,0) ), dot( v110, f - vec3(1,1,0) ), u.x );
    float c = lerp1( dot( v001, f - vec3(0,0,1) ), dot( v101, f - vec3(1,0,1) ), u.x );
    float d = lerp1( dot( v011, f - vec3(0,1,1) ), dot( v111, f - vec3(1,1,1) ), u.x );
